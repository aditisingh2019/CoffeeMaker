...
public class APIRecipeController extends APIController {
     ...
     * recipe to be updated (as a path variable)
     *
     * @param r
     *            The ingredient we need to update
     * @return Success if the ingredient could be updated
     */
    @PutMapping ( BASE_PATH + "/recipes" )
    public ResponseEntity updateRecipe ( @RequestBody final Recipe r ) {
        final Recipe recipe = service.findByName( r.getName() );
        recipe.updateRecipe( r );
        service.save( recipe );

        return new ResponseEntity( successResponse( recipe.getName() + " was successfully updated" ), HttpStatus.OK );
    }
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( null != service.findByName( recipe.getName() ) ) {
            // check if this recipe name is in the db -> if it exists return
        ...
        if ( service.findAll().size() < 3 ) {
            // then check if the number of recipes in the database is less than
            // 3 -> if true add else throw exception
            ...
            for ( int i = 0; i < recipe.getIngredients().size(); i++ ) {
                ...
                ing.setName( recipe.getIngredients().get( i ).getName() );
                ing.setAmount( recipe.getIngredients().get( i ).getAmount() );
                r.addIngredient( ing );
            }
            service.save( r );
